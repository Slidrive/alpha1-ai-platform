# Alpha1 AI Platform - IAM Roles and Policies
...
# Additional policy for EKS nodes to access S3 and other AWS services
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/alpha1-ai-platform/*"
      }
    ]
  })
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks[0].url, "https://", "")}:sub" = "system:serviceaccount:default:alpha1-app"
            "${replace(aws_iam_openid_connect_provider.eks[0].url, "https://", "")}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
resource "aws_iam_role_policy_attachment" "app_service_account_s3" {
  policy_arn = aws_iam_policy.eks_nodes_additional.arn
